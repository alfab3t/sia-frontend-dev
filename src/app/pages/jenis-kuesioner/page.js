...
export default function MasterJenisKuesionerPage() {
    const ssoData = useMemo(() => getSSOData(), []);
    const userData = useMemo(() => getUserData(), []);
    const router = useRouter();
    const [dataJenisKuesioner, setDataJenisKuesioner] = useState([]);
    ...
    const dataFilterSort = [
        { Value: "[Nama Jenis Kuesioner] asc", Text: "Nama Jenis [↑]" },
        { Value: "[Nama Jenis Kuesioner] desc", Text: "Nama Jenis [↓]" },
    ];
        ...
        async (page, sort, cari, status) => {
            ...
            try {
                const response = await fetchData(
                    API_LINK + "JenisKuesioner/GetAllJenisKuesioner",
                    {
                        Status: status,
                ...
                const pagedData = data.map((item, index) => ({
                    ...
                    id: item.id,
                    "Nama Jenis": item.namaJenis,
                    Status: item.status,
                    Aksi: [
                        ...(isClient && userData?.permission?.includes("jenis_kuesioner.edit")
                            ? ["Toggle", "Edit"]
                            : []),
                    ],
                    Alignment: [
                ...
                setDataJenisKuesioner(pagedData || []);
                setTotalData(totalData || 0);
                setCurrentPage(page);
            } catch (err) {
                Toast.error(err.message);
                setDataJenisKuesioner([]);
                setTotalData(0);
            } finally {
                setLoading(false);
            }
    ...
    const handleAdd = useCallback(() => {
        router.push("/pages/kuesioner/jenis-kuesioner/add");
    }, [router]);

    const handleEdit = useCallback((id) => {
        router.push(`/pages/kuesioner/jenis-kuesioner/edit/${encryptIdUrl(id)}`);
    }, [router]);

    const handleToggle = useCallback(
        async (id) => {
            const item = dataJenisKuesioner.find((d) => d.id === id);
            if (!item) {
            ...
            try {
                ...
                const data = await fetchData(
                    API_LINK + "JenisKuesioner/SetStatusJenisKuesioner",
                    payload,
                    "POST"
                );
        ...
        [dataJenisKuesioner, currentPage, sortBy, search, sortStatus, loadData]
    );
        ...
        >
            <div>
                <Formsearch
                    ...
                    showAddButton={isClient && userData?.permission?.includes("jenis_kuesioner.create")}
                    showExportButton={false}
                    searchPlaceholder="Cari jenis kuesioner.."
